    ...
    for i in xrange(1,Nf-1):
        ...
        if dydx*symbol < 0.0:
            if abs(dydx) > knon:
                ...
                symbol = -1.0*symbol
    if ncount > 1:
        return False, airfoil
    else:
        return True, airfoil

os.system('mkdir picked_uiuc')

fname=open('step3_name.txt','w')
fdis = open('step3_discarded.txt','w')
...
for iair in xrange(len(namelist)):
    ...
    if pickflag:
        ...
        f = open('picked_uiuc/'+filename,'w')
        for i in range(newairfoil.shape[0]):
            f.write('%.15f %.15f\n'%(newairfoil[i,0],newairfoil[i,1]))
        f.close()
